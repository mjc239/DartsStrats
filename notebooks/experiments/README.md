...
| **20** What couples the darts | 19 says the darts are not independent. What replaces the assumption? | **Mostly not the throw.** Professionals use **four** scoring targets and step down them after a miss — from the 20, a miss moves to the 19 24.8% of the time; from the 19 to the 18, 35.7%. That alone accounts for about half the coupling 19 measured, and the model has no state for it. Second: a single dart's Gaussian tails are far too thin — a wide component on ~9% of darts is worth **5.4 log-likelihood units a visit**, against 0.06 for everything to do with dependence. What is left is a shared **scale** (the visit is tight or loose), not a shared **offset** (a stance) — it wins on 15 of 19 players. It is still not the right mechanism: it overshoots the magnitude signature four-fold and leaves the direction coupling unexplained |
